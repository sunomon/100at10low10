...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(hours=1):
            ...
            target7_price = get_target7_price("KRW-CHZ", 1)
            target8_price = get_target8_price("KRW-CHZ", 1.5)
            target9_price = get_target9_price("KRW-CHZ", 2)
            target10_price = get_target10_price("KRW-CHZ", 3)
            ...
            chz = get_balance("CHZ")
            current_price = get_current_price("KRW-CHZ")
            if chz > 1000000*1.001*1.05/current_price:
                upbit.sell_market_order("KRW-CHZ", chz*0.9995)  
        elif middle3_time < now < end_time - datetime.timedelta(hours=1):
            chz = get_balance("CHZ")
            current_price = get_current_price("KRW-CHZ")
            if chz > 1000000*1.001*1.03/current_price or current_price > predicted_close_price:
                upbit.sell_market_order("KRW-CHZ", chz*0.9995)  
        else:
            chz = get_balance("CHZ")
            current_price = get_current_price("KRW-CHZ")
