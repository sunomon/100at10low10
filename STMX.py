...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(hours=1):
            ...
            target7_price = get_target7_price("KRW-STMX", 1)
            target8_price = get_target8_price("KRW-STMX", 1.5)
            target9_price = get_target9_price("KRW-STMX", 2)
            target10_price = get_target10_price("KRW-STMX", 3)
            ...
            stmx = get_balance("STMX")
            current_price = get_current_price("KRW-STMX")
            if stmx > 1000000*1.001*1.05/current_price:
                upbit.sell_market_order("KRW-STMX", stmx*0.9995)  
        elif middle3_time < now < end_time - datetime.timedelta(hours=1):
            stmx = get_balance("STMX")
            current_price = get_current_price("KRW-STMX")
            if stmx > 1000000*1.001*1.03/current_price or current_price > predicted_close_price:
                upbit.sell_market_order("KRW-STMX", stmx*0.9995)  
        else:
            stmx = get_balance("STMX")
            current_price = get_current_price("KRW-STMX")
