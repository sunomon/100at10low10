...
predict_price("KRW-POLY")
schedule.every().hour.do(lambda: predict_price("KRW-POLY"))
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(hours=1):
            target1_price = get_target1_price("KRW-POLY", 0.1)
            target2_price = get_target2_price("KRW-POLY", 0.2)
            target3_price = get_target3_price("KRW-POLY", 0.3)
            target4_price = get_target4_price("KRW-POLY", 0.4)
            target5_price = get_target5_price("KRW-POLY", 0.5)
            target6_price = get_target6_price("KRW-POLY", 0.6)
            target7_price = get_target7_price("KRW-POLY", 1)
            target8_price = get_target8_price("KRW-POLY", 1.5)
            target9_price = get_target9_price("KRW-POLY", 2)
            target10_price = get_target10_price("KRW-POLY", 3)
            ...
            if target1_price <= current_price < target1_price*1.02 and target1_price*1.1 <= predicted_close_price:
                if krw >= 1000000 and poly < 10000/(target1_price*1.02):
                    upbit.buy_market_order("KRW-POLY", 1000000)
                if 5000 < krw < 1000000 and poly < 10000/(target1_price*1.02):
            ...
            if target2_price <= current_price < target2_price*1.02 and target2_price*1.125 <= predicted_close_price:
                if krw >= 1000000 and poly < 10000/(target2_price*1.02):
                    upbit.buy_market_order("KRW-POLY", 1000000)
                if 5000 < krw < 1000000 and poly < 10000/(target2_price*1.02):
                    upbit.buy_market_order("KRW-POLY", krw*0.9995)
            if target3_price <= current_price < target3_price*1.02 and target3_price*1.15 <= predicted_close_price:
                if krw >= 1000000 and poly < 10000/(target3_price*1.02):
                    upbit.buy_market_order("KRW-POLY", 1000000)
                if 5000 < krw < 1000000 and poly < 10000/(target3_price*1.02):
                    upbit.buy_market_order("KRW-POLY", krw*0.9995)
            if target4_price <= current_price < target4_price*1.02 and target4_price*1.175 <= predicted_close_price:
                if krw >= 1000000 and poly < 10000/(target4_price*1.02):
                    upbit.buy_market_order("KRW-POLY", 1000000)
                if 5000 < krw < 1000000 and poly < 10000/(target4_price*1.02):
                    upbit.buy_market_order("KRW-POLY", krw*0.9995)
            if target5_price <= current_price < target5_price*1.02 and target5_price*1.2 <= predicted_close_price:
                if krw >= 1000000 and poly < 10000/(target5_price*1.02):
                    upbit.buy_market_order("KRW-POLY", 1000000)
                if 5000 < krw < 1000000 and poly < 10000/(target5_price*1.02):
                    upbit.buy_market_order("KRW-POLY", krw*0.9995)
            if target6_price <= current_price < target6_price*1.02 and target6_price*1.25 <= predicted_close_price:
                if krw >= 1000000 and poly < 10000/(target6_price*1.02):
                    upbit.buy_market_order("KRW-POLY", 1000000)
                if 5000 < krw < 1000000 and poly < 10000/(target6_price*1.02):
                    upbit.buy_market_order("KRW-POLY", krw*0.9995)
            if target7_price <= current_price < target7_price*1.02 and target7_price*1.2 <= predicted_close_price:
                if krw >= 1000000 and poly < 10000/(target7_price*1.02):
                    upbit.buy_market_order("KRW-POLY", 1000000)
                if 5000 < krw < 1000000 and poly < 10000/(target7_price*1.02):
                    upbit.buy_market_order("KRW-POLY", krw*0.9995)
            if target8_price <= current_price < target8_price*1.02 and target8_price*1.3 <= predicted_close_price:
                if krw >= 1000000 and poly < 10000/(target8_price*1.02):
                    upbit.buy_market_order("KRW-POLY", 1000000)
                if 5000 < krw < 1000000 and poly < 10000/(target8_price*1.02):
                    upbit.buy_market_order("KRW-POLY", krw*0.9995)
            if target9_price <= current_price < target9_price*1.02 and target9_price*1.4 <= predicted_close_price:
                if krw >= 1000000 and poly < 10000/(target9_price*1.02):
                    upbit.buy_market_order("KRW-POLY", 1000000)
                if 5000 < krw < 1000000 and poly < 10000/(target9_price*1.02):
                    upbit.buy_market_order("KRW-POLY", krw*0.9995)
            if target10_price <= current_price < target10_price*1.02 and target10_price*2 <= predicted_close_price:
                if krw >= 1000000 and poly < 10000/(target10_price*1.02):
                    upbit.buy_market_order("KRW-POLY", 1000000)
                if 5000 < krw < 1000000 and poly < 10000/(target10_price*1.02):
                    upbit.buy_market_order("KRW-POLY", krw*0.9995)
            if poly > 1000000*1.001*1.2/current_price:
                upbit.sell_market_order("KRW-POLY", poly*0.9995)  
        elif middle1_time < now < middle2_time:
            poly = get_balance("POLY")
            current_price = get_current_price("KRW-POLY")
            if poly > 1000000*1.001*1.1/current_price:
                upbit.sell_market_order("KRW-POLY", poly*0.9995)  
        elif middle2_time < now < middle3_time:
            poly = get_balance("POLY")
            current_price = get_current_price("KRW-POLY")
            if poly > 1000000*1.001*1.05/current_price:
                upbit.sell_market_order("KRW-POLY", poly*0.9995)  
        elif middle3_time < now < end_time - datetime.timedelta(hours=1):
            poly = get_balance("POLY")
            current_price = get_current_price("KRW-POLY")
            if poly > 1000000*1.001*1.03/current_price or current_price > predicted_close_price:
                upbit.sell_market_order("KRW-POLY", poly*0.9995)  
        else:
            poly = get_balance("POLY")
            current_price = get_current_price("KRW-POLY")
            if poly > 1000000*1.001/current_price:
                upbit.sell_market_order("KRW-POLY", poly*0.9995)
        time.sleep(1)
