...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(hours=1):
            ...
            target7_price = get_target7_price("KRW-MOC", 1)
            target8_price = get_target8_price("KRW-MOC", 1.5)
            target9_price = get_target9_price("KRW-MOC", 2)
            target10_price = get_target10_price("KRW-MOC", 3)
            ...
            moc = get_balance("MOC")
            current_price = get_current_price("KRW-MOC")
            if moc > 1000000*1.001*1.05/current_price:
                upbit.sell_market_order("KRW-MOC", moc*0.9995)  
        elif middle3_time < now < end_time - datetime.timedelta(hours=1):
            moc = get_balance("MOC")
            current_price = get_current_price("KRW-MOC")
            if moc > 1000000*1.001*1.03/current_price or current_price > predicted_close_price:
                upbit.sell_market_order("KRW-MOC", moc*0.9995)  
        else:
            moc = get_balance("MOC")
            current_price = get_current_price("KRW-MOC")
