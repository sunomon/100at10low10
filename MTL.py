...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(hours=1):
            ...
            target7_price = get_target7_price("KRW-MTL", 1)
            target8_price = get_target8_price("KRW-MTL", 1.5)
            target9_price = get_target9_price("KRW-MTL", 2)
            target10_price = get_target10_price("KRW-MTL", 3)
            ...
            mtl = get_balance("MTL")
            current_price = get_current_price("KRW-MTL")
            if mtl > 1000000*1.001*1.05/current_price:
                upbit.sell_market_order("KRW-MTL", mtl*0.9995)  
        elif middle3_time < now < end_time - datetime.timedelta(hours=1):
            mtl = get_balance("MTL")
            current_price = get_current_price("KRW-MTL")
            if mtl > 1000000*1.001*1.03/current_price or current_price > predicted_close_price:
                upbit.sell_market_order("KRW-MTL", mtl*0.9995)  
        else:
            mtl = get_balance("MTL")
            current_price = get_current_price("KRW-MTL")
